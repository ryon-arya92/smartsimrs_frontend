...
const Ranap = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get("/api/proxy/ranap");
  ...
  const handleDetailClick = async (id) => {
    try {
      ...
      const [resRegister, resTransaksi] = await Promise.all([
        api.post("/api/get/registerranap", { idRegisterKunjungan: id }),
        api.post("/api/get/transaksiranap", { idRegisterKunjungan: id }),
      ]);
  ...
  const handleDeleteRegister = async (row) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await api.delete(`/api/proxy/ranap/delete-register/${row.idRegisterKunjungan}`);
        ...
        <Card className={theme === "dark" ? "bg-dark text-white" : ""}>
          <Card.Header className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-3 bg-transparent">
            <div className="header-title">
              <h4 className="card-title mb-0">Data Register Ranap</h4>
            </div>
